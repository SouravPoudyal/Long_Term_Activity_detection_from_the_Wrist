...
def load_dataset(dataset, pred_type='actions', include_null=False):
    ...
    if dataset == 'wetlab':
        ...
        sampling_rate = 100
        has_null = True
    elif dataset == 'basketball_':
        if pred_type == 'basketball':
            class_names = ['shot', 'layup', 'dribbling', 'rebound', 'pass']
        elif pred_type == 'locomotion':
            class_names = ['running', 'sitting', 'standing', 'walking']
        sampling_rate = 50
        has_null = True
    ...
    data = pd.read_csv(os.path.join('csv_/locomotion', dataset + '.csv'), sep=',', header=None, index_col=None)
    X, y = preprocess_data(data, dataset, pred_type, has_null, include_null)

    print(" ..from file {}".format(os.path.join('csv_/locomotion', dataset + '.csv')))
...
def preprocess_data(data, ds, pt='actions', has_null=False, include_null=True):
    ...
    if (ds == 'wetlab' and pt == 'actions') or ((ds == 'opportunity' or ds == 'opportunity_ordonez') and pt == "locomotion"):
        ...
        #print(data)
        X, y = data.iloc[1:, :-4], adjust_labels(data.iloc[1:, -3], ds, pt).astype(int)
...
def adjust_labels(data_y, dataset, pred_type='actions'):
    ...
    if dataset == 'wetlab':
        ...
        if pred_type == 'basketball':
            data_y[data_y == "shot"] = 1
            data_y[data_y == "layup"] = 2
            data_y[data_y == "dribbling"] = 3
            data_y[data_y == "rebound"] = 4
            data_y[data_y == "pass"] = 5
            #class_names = ['shot', 'layup', 'dribbling', 'rebound', 'pass']
        elif pred_type == 'locomotion':
            #data_y[data_y == "not_labelled"] = 0
            data_y[data_y == "running"] = 1
            data_y[data_y == "sitting"] = 2
            data_y[data_y == "standing"] = 3
            data_y[data_y == "walking"] = 4
            #class_names = ['running', 'sitting', 'standing', 'walking']
    elif dataset == 'hhar':
